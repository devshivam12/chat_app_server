...
const login = TryCatch(async (req, res, next) => {
    ...
    if (!isMatch) return next(new ErrorHandler("Invalid UserName or Password"))
...
const logout = TryCatch(async (req, res) => {
    res.status(200).cookie("chattenger", "", { ...cookieOption, maxAge: 0 }).json({
        success: true,
...
const searchUser = TryCatch(async (req, res) => {
    ...
    // extrting all users from my chats means friends or people i have chatted with 

    const allUserFromMyChats = myChats.flatMap((chat) => chat.members)

    //finding all users axcept me and my friends
